// -----------------------------------------------------------------------------
//
// rlm.js - the base class for parsing RLM license files.
//
...
var rlm = function rlm (filename) {
  ...
  for (i=0; i<lines.length; i++) {
    ...
    if ( line.trim().length > 0 ) {  // Skip comments and empty lines
      if (firstchar !== ' ') {
        _licarr.push(line);
      } else {
        line = line.substr(2);
        _licarr[_licarr.length-1] = _licarr[_licarr.length-1] +
                                    (((_licarr[_licarr.length-1].replace(/[^"]/g, '').length % 2 ) === 1) ? '' : ' ')
                                    + line;
      }
    }
  }
...
//ISV isvname [isv-binary-pathname [options-file-filename [port-number]]] [binary=isvbinary-pathname] [options=options-file-filename] [port=port-number] [password=password-text]
  ...
  for (i = 0; i < line.length; i++) {
    ...
    if (line[i] !== '') {
      if (line[i].toLowerCase().indexOf('port=') !== -1) {
        ...
        positional=false;
        c = 3;
      } else if (line[i].toLowerCase().indexOf('binary=') !== -1) {
        value = line[i].substr(line[i].toLowerCase().indexOf('binary=')+7);
        positional=false;
        c = 1;
      } else if (line[i].toLowerCase().indexOf('password=') !== -1) {
        value = line[i].substr(line[i].toLowerCase().indexOf('password=')+9);
        positional=false;
        c = 4;
...
function parseFeature(line) {
  ...
  if (line.length > 1) {
    feature.vendor = line[1] || null;
    feature.name = line[2] || null;
    feature.version = line[3] || null;
    feature.expDate = line[4] || null;
    feature.licCount = line[5] || null;
    if ((feature.licCount !== null) && (feature.licCount === '0')) {
      feature.licCount = 'uncounted';
    }
    if ((feature.expDate !== null) && (feature.expDate.toLowerCase() !== 'permanent')) {
      var darr = feature.expDate.split('-');
      if ((darr.length === 3) && ((darr[2] === '0') || (darr[2] === '0000'))) {
        feature.expDate = 'permanent';
      }
    }
    var kv;
    var featurename = '';
    ...
    for (var i=6; i< line.length; i++) {
      if (line[i].indexOf('=') >= 0) {
        ...
        joinnext = (line[i].replace(/[^"]/g, '').length === 1);
      } else {
        if (joinnext === false) {
          featurename = line[i];
          ...
          joinnext = (line[i].replace(/[^"]/g, '').length === 0);
        }
      }
    }
    return feature;
